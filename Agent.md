...
**الحالة الحالية:** Phase 0–12 اكتملت. آخر مرحلة منجزة: **Phase 12 — Settings & Backup** (إعدادات المدرسة بـ Logo dropzone + إدارة السنوات الدراسية + المستخدمون والأدوار مع BCrypt + حماية آخر admin + النسخ الاحتياطي والاسترجاع عبر `Microsoft.Data.SqlClient` خام مع `IsSafeDatabaseName` whitelist + Application.Shutdown بعد RESTORE). المرحلة التالية هي **Phase 13 — Final Polish, Splash, Setup Wizard, and Testing** (Splash Screen يطبّق Migrations تلقائياً + First-Run Setup Wizard لإعداد Connection String + polish RTL شامل + اختبار end-to-end + README للمستخدم النهائي). لا تبدأ Phase 13 دون طلب صريح من المستخدم.
